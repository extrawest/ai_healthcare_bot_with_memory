...
def main():
    ...
    cmd = ["chainlit", "run", str(chainlit_app_path), "--host", settings.host, "--port", str(settings.ui_port)]

    logger.info(f"Starting Chainlit UI at http://{settings.host}:{settings.ui_port}")
